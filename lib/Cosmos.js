...
const CosmosDidDoc = require('./CosmosDidDoc');
...
class Cosmos {
  ...
  constructor(options = {}) {
    ...
    const hostname = options.hostname || Cosmos.defaultHostname(this.mode);
  ...
  static defaultHostname(mode) {
    switch(mode) {
      ...
      case 'test':
        return 'localhost:1317/did/identifier';
      case 'live':
        return 'veres.one';
      default:
   ...
   * @param [options.authDoc] {CosmosDidDoc} Auth DID Doc, required if using
  ...
  async computeKeyId({
    ...
    if(!did) {
      did = CosmosDidDoc.generateDid({key, didType, mode});
    }
    return CosmosDidDoc.generateKeyId({did, keyPair: key});
  }
  ...
  async get({did, keys, forceConstruct = false, autoObserve = false}) {
    ...
    const didDoc = new CosmosDidDoc(result);
   ...
   * @param options {object}
   *
   * @param options.didDocument {CosmosDidDoc} Document to update
   *
   * @param [options.accelerator] {string} Hostname of accelerator to use
   * @param [options.authDoc] {CosmosDidDoc} Auth DID Doc, required if using
   *   an accelerator service
...
Cosmos.contexts = {
  [constants.VERES_ONE_CONTEXT_URL]:
    cosmosContext.contexts.get(constants.VERES_ONE_CONTEXT_URL)
};
