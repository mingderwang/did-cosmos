...
const VERES_DID_REGEX = /^(did:cosm:)(test:)?(uuid|nym):(.+)/;
const didContexts = [
...
class CosmosDidDoc {
  ...
  static async generate(options) {
    ...
    const did = new CosmosDidDoc(options);
  ...
  static async fromNym({did}) {
    ...
    return await CosmosDidDoc.generate({
      mode,
  ...
  static generateDid({
    ...
    if(didType === 'uuid') {
      const prefix = (mode === 'test') ? 'did:cosm:test:' : 'did:cosm:';
    ...
    return CosmosDidDoc.createCryptonymDid({key, mode});
  }
  ...
  async init({
    ...
    const did = CosmosDidDoc.generateDid({key: invokeKey, didType, mode});
    this.doc.id = did;
    ...
    for(const keyPair of [authKey, invokeKey, delegateKey, assertionKey]) {
      ...
      if(!keyPair.id) {
        keyPair.id = CosmosDidDoc.generateKeyId({did, keyPair});
      }
    }
  ...
  async rotateKey({key, id, passphrase}) {
    ...
    newKey.id = CosmosDidDoc.generateKeyId({did: this.id, keyPair: newKey});
