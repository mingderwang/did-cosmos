...
const CosmosClientError = require('./CosmosClientError');
...
class CosmosClient {
  ...
  async get({did, forceConstruct = false}) {
    ...
    const isNym = (did.startsWith('did:cosm:nym:') ||
      did.startsWith('did:cosm:test:nym:'));
    let result = {};
    let didDoc;

    // FIXME - remove, replace with cache
    if(isNym && forceConstruct) {
      didDoc = (await CosmosDidDoc.fromNym({did: docUri})).doc;
    } else {
      try {
  ...
  async getTicketServiceProof({operation, ticketService}) {
    ...
    if(response.problem) {
      const error = new CosmosClientError(
  ...
  async sendToAccelerator(options) {
    ...
    await CosmosClient.signRequestHeaders({
      requestOptions: {
        url: acceleratorUrl,
        method: 'POST',
        headers
